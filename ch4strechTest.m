...
fct = cell(0,0);
load(['C:\Users\mtanha\MSQC\msqc\factoryslater\ch4rDat\all-3.mat'],'fact');
...
fct{1} = fact;
%%
color={'b-','r-','g-','k-','m-','y-','c-','b-','g-','k-','m-','y-'};
etot1= [];
etot2= [];
for idata = 1:length(dataf);
    f1 = fct{1}.makeFitme(dsets{idata});
    ...
    for i = 1:12
        ...
        for i2 = 1:ngeom
            ic = ic+1;
            if idata == 1
            etotEnv(ic) =627.509*etot1(nmb(i2,i));
            extmax = 15;
            elseif idata == 2
            etotEnv(ic) =627.509*etot2(nmb(i2,i));
            extmax = 10;
            end
        end
        figure(idata*100)
    ...
    ELL = [];
    for i2 =1:ngeom
    EHL(i2) = f1.HLKE{i2}(12)+sum(f1.HLEN{i2}(12))+f1.HLE2{i2}(12);
    ELL(i2) =  f1.LLKE{i2}(12)+sum(f1.LLEN{i2}(12))+f1.LLE2{i2}(12);
    end
    EHL = 627.509*(EHL-min(EHL));
    ELL = 627.509*(ELL-min(ELL));
        figure((idata+5)*100)
        plot(ext(1:extmax),etotEnv,'b-');
        hold on
